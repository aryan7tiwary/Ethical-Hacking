...
def extractData(data):    
    ...
    for match in matches:
        if len(match) == 0:
            continue
        try:
            if not len(match) % 4 == 0:             # Checking if '=' signs have been removed from base64 data since base64 is always in multiple of 4
                padnum = (4-len(match)%4)%4         # Adding back '=' signs, i.e, padding
                match += b"=" * padnum
